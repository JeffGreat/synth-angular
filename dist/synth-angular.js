 ...
 * 
 */
  listDirective.directive('syList', [
    '$uiManager',
    'dialogs',
    '$state',
    function ($uiManager, $dialogs, $state) {
      return {
        restrict: 'AE',
        transclude: true,
        scope: {
          ...
          $scope.launch = function (which) {
            ...
            case 'create':
              ...
              dlg.result.then(function (element) {
                $scope.confirmed = 'L\'\xe9l\xe9ment a bien \xe9t\xe9 ajout\xe9';
  ...
  listDirective.directive('syListCell', [
    '$compile',
    'dialogs',
    '$uiManager',
    '$state',
      ...
      return {
        ...
        controller: function ($scope) {
          $scope.launch = function (which) {
            ...
            switch (which) {
            case 'confirm':
              dlg = $dialogs.confirm('Demande de confirmation', 'Etes-vous s\xfbr de vouloir supprimer l\'\xe9l\xe9ment suivant: <h3>' + $scope.item[$scope.$parent.$parent.$parent.nameIdentifiant] + '</h3>');
              dlg.result.then(function (btn) {
                $scope.deleteElement($scope.item);
                $scope.$parent.$parent.$parent.confirmed = 'L\'\xe9l\xe9ment a bien \xe9t\xe9 supprim\xe9';
              }, function (btn) {
                $scope.$parent.$parent.$parent.confirmed = 'La suppression a \xe9t\xe9 avort\xe9e';
              });
              break;
            case 'edit':
